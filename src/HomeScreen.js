...
//   const reopened = () => {
//     const openData = firestore().collection('CreateTask').where('Taskstatus', '==', 'Reopened').get().then(function (querySnapshot) {
  ...
  return (
    <View>
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={HomeScreenStyles.mainView}>
            <View style={HomeScreenStyles.newBorder}>
              <Text style={HomeScreenStyles.text1}>{item.name}</Text>
              <Text style={HomeScreenStyles.text2}>{item.num}</Text>
            </View>
          </View>
        )}
      />
